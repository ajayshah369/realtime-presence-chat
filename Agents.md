...
Serverless real-time chat/presence backend. AWS API Gateway WebSocket API → Lambda (Node.js) → DynamoDB, deployed via AWS CDK (JavaScript, not TypeScript). No traditional server; connection state lives in DynamoDB because Lambda has no memory between invocations.
...
There is no local dev server or offline emulator configured for this project — testing happens against real deployed AWS resources (see "Testing" below). Do not assume `npm start` or similar exists.

## Code style

- Plain JavaScript (CommonJS, `require`/`module.exports`), not TypeScript, across both `cdk/` and `lambda/`.
...
- **`connect.js`, `disconnect.js`, `default.js`, and `sendMessage.js` all share one zipped asset** (`lambda.Code.fromAsset('../../lambda')` in `cdk/lib/realtime-dashboard-stack.js`). Adding a new file to `lambda/` does not require a new `fromAsset` call — just reference the new file's handler in a new `lambda.Function` construct.
...
There are no automated tests in this repo yet. Manual verification flow after any change to `lambda/` or the CDK stack:

1. `cdk deploy`
2. Connect two clients (either `wscat -c "wss://<url>/prod?username=X"` in two terminals, or `client/index.html` in two browser tabs)
