...
- If asked to add a new Lambda route, remember the three-part checklist from AGENTS.md: (1) new file in `backend/lambda/`, (2) new `lambda.Function` construct + grants in `backend/cdk/lib/realtime-dashboard-stack.js`, (3) new `webSocketApi.addRoute(...)` call. Missing any one of these is the most common source of "it deploys but doesn't work" bugs in this codebase.
